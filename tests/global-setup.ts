...
import { startCluster, bootstrapRoleAndDb, APP_DB, APP_USER, APP_PASSWORD } from "../scripts/pg.mjs";
...
export default async function setup({ provide }: { provide: (key: "orgAId" | "orgBId", value: string) => void }) {
  // Always start from a clean cluster (a prior crashed run may have left the dir behind).
  rmSync(DATA_DIR, { recursive: true, force: true });

  const cluster = await startCluster({ port: PORT, dataDir: DATA_DIR, persistent: false, quiet: true });
