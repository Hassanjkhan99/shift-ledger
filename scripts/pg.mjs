...
export async function bootstrapRoleAndDb({ port }) {
  ...
  try {
    ...
    if (role.rowCount === 0) {
      ...
      await admin.query(`CREATE ROLE ${APP_USER} LOGIN PASSWORD '${APP_PASSWORD}' NOSUPERUSER CREATEDB NOCREATEROLE NOBYPASSRLS`);
    }
  ...
  const appdb = new pg.Client({ host: "localhost", port, user: "postgres", password: "postgres", database: APP_DB });
  await appdb.connect();
  try {
